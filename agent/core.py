...
from .roles import planner, coder, reviewer, fixer
from .tester import generate_tests, write_test_file, run_tests
...
def update_context(context: str, new_code: str, max_chars: int = 12000):
    ...
    Keeps most recent content only.
    """
    updated = context + "\n\n" + new_code
...
def run_agent(task: str):
    ...
    for i, step in enumerate(state["plan"]):
        ...
        # Generate tests
        tests = safe_call(generate_tests, code, fallback="")
        if tests:
